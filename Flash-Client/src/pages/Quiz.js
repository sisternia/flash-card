...
const Quiz = () => {
  ...
  useEffect(() => {
    ...
    const fetchFlashcards = async () => {
      ...
      setTimeLeft(data.length * 5); // 5 giây mỗi từ
  ...
  const handleDrop = (targetId) => {
    ...
    if (dragId === targetId) {
      ...
      setPairs(currentPairs =>
        currentPairs.map(p =>
          p.id === targetId ? { ...p, matched: true, droppedBack: options.find(o => o.id === dragId)?.back } : p
        )
      );
      setOptions(currentOptions =>
        currentOptions.map(o => (o.id === dragId ? { ...o, matched: true } : o))
      );
      setResult(r => ({ ...r, correct: r.correct + 1 }));

      // Kiểm tra hoàn thành
      if (result.correct + 1 === result.total) {
        clearInterval(timerRef.current);
        setResult(r => ({ ...r, finished: true }));
      }
    } else {
      // Khi ghép sai, chỉ reset trạng thái kéo thả, không hiển thị gì cả
      setDropTarget(null); // Bỏ highlight
    }

    setDragId(null); // Reset trạng thái kéo
  };

  // Thanh progress bar thời gian
  const renderProgressBar = () => (
    <div style={{ width: '100%', height: 16, background: '#e0e0e0', borderRadius: 8, marginBottom: 18 }}>
      <div style={{ width: `${Math.max(0, (timeLeft / (flashcards.length * 5)) * 100)}%`, height: '100%', background: '#2ecc40', borderRadius: 8, transition: 'width 0.5s' }} />
    </div>
  );
              ...
              <div className="dragdrop-main">
                <div className="dragdrop-pairs">
                  {pairs.map(pair => (
                    <div
                      key={pair.id}
                      className={`dragdrop-dropzone${pair.matched ? ' matched' : ''}`}
                    ...
                    >
                      ...
                      {pair.matched && <span className="dragdrop-back">{pair.droppedBack}</span>}
                    </div>
                  ))}
                </div>
